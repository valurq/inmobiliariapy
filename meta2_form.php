        ...
        */
        if(isset($_POST['seleccionado'])){
            ...
            */
            ...
            $test=$inserta_Datos->consultarDatos( array('ano'),'metas', "","oficina_id", $oficina );
            $anhos=array();
            while($aux=$test->fetch_row()){
              array_push($anhos, $aux[0]);
            }
    ?>
...
<body style="background-color:white">
  ...
  <form action="meta_panel.php" method="POST" id='form_contrato'>
...
<script type="text/javascript">
    var anhos = JSON.parse('<?php  echo json_encode($anhos) ?>');
    var anho = "<?php 
    if(count($resultado) > 1) {echo $resultado[2];} else {echo 'undefined';} ?>";
...
//======================================================================
    ...
    if(anho != $("#ano").val()){
      for (var i = 0; i < anhos.length; i++) {
        if ($("#ano").val() == anhos[i]) {
          popup('Advertencia','El año de una meta no puede repetirse!!') ;
          return false;
        }
      }
    }
